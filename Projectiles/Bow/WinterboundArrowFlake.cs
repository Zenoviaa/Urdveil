...
namespace Urdveil.Projectiles.Bow
{
    internal class WinterboundArrowFlake : ModProjectile
    {
        ...
        {
            Texture2D snowflakeTexture = ModContent.Request<Texture2D>("Urdveil/Particles/SnowFlakeParticleSmall").Value;
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Texture2D dimLightTexture = ModContent.Request<Texture2D>("Urdveil/Assets/NoiseTextures/DimLight").Value;
            float drawScale = 1f;
            SpriteBatch spriteBatch = Main.spriteBatch;
            for (int i = 0; i < 2; i++)
            {
