...
namespace Urdveil.Projectiles.Magic
{
    ...
    internal class BrincShot : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/SoftSummon2"), Projectile.position);
            }
            if (alphaCounter <= 1)
            {
                alphaCounter += 0.08f;
            ...
            if (Projectile.ai[0] == 2)
            {
                ...
                if (Sound == 2)
                {
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Morrowarrow"), Projectile.position);
                }
                if (Sound == 3)
                {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustID.BoneTorch, (Vector2.One * Main.rand.Next(1, 12)).RotatedByRandom(25.0), 0, default(Color), 0.6f).noGravity = true;
            }
            for (int i = 0; i < 2; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustID.BoneTorch, (Vector2.One * Main.rand.Next(1, 12)).RotatedByRandom(25.0), 0, default(Color), 0.2f).noGravity = false;
            }
        ...
        {
            Texture2D texture2D4 = Request<Texture2D>("Urdveil/Assets/NoiseTextures/DimLight").Value;
