...
namespace Urdveil.Projectiles.Magic
{
    ...
    internal class ITProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                SoundEngine.PlaySound(new SoundStyle($"{nameof(Urdveil)}/Assets/Sounds/IrradiatedNest_Fall"), Projectile.position);
                Projectile.spriteDirection = Projectile.direction;
                Moved = true;
            }
            if (Projectile.ai[1] == 30)
            {
                SoundStyle soundStyle = new SoundStyle("Urdveil/Assets/Sounds/ITBeep");
                //Between -1 and 1f
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (Projectile.Colliding(myRect, otherRect))
                {
                    ...
                    {
                        SoundEngine.PlaySound(new SoundStyle($"Urdveil/Assets/Sounds/ITPrimer"), Projectile.position);
                        Projectile.ai[1] = 111;
                    }
                }
            }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (S1 == 0)
            {
                SoundEngine.PlaySound(new SoundStyle($"{nameof(Urdveil)}/Assets/Sounds/ITBomb1"), Projectile.position);
            }
            if (S1 == 1)
            {
                SoundEngine.PlaySound(new SoundStyle($"{nameof(Urdveil)}/Assets/Sounds/ITBomb2"), Projectile.position);
            }
            if (S1 == 2)
            {
                SoundEngine.PlaySound(new SoundStyle($"{nameof(Urdveil)}/Assets/Sounds/ITBomb3"), Projectile.position);
            }
