...
namespace Urdveil.Projectiles.Thrown
{
    public class ThePenetratorProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            switch (State)
            {
                case ActionState.Spin:
                    if (Timer == 0)
                    {
                        SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsSlashCharge"), Projectile.position);
                    }
                    ...
                    if (Timer < Charge_Time)
                    {
                        ...
                        if (Timer >= Charge_Time)
                        {
                            SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/GladiatorMirage1"), Projectile.position);
                        }
                    }
                    ...
                    if (Sound % 15 == 0)
                    {
                        SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/SkyrageShasher") { Pitch = SoundPitch }, Projectile.position);
                        SoundPitch += 0.03f;
                    }
                    ...
                    if (Main.myPlayer == Projectile.owner)
                    {
                        if (!player.channel)
                        {
                            ...
                            {
                                case 0:
                                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsSlashProj2"), Projectile.position);
                                    break;
                                case 1:
                                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsSlashProj3"), Projectile.position);
                                    break;
                                case 2:
                                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsSlashProj4"), Projectile.position);
                                    break;
                            }
                        }
                    }
        ...
        {
            var TrailTex = ModContent.Request<Texture2D>("Urdveil/Assets/NoiseTextures/StarTrail").Value;
            ...
            Texture2D spinTexture = ModContent.Request<Texture2D>("Urdveil/Assets/NoiseTextures/Spiin").Value;
            for (int i = 0; i < 2; i++)
            {
