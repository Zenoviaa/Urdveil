...
namespace Urdveil.Projectiles.Thrown
{
    public class CleanestCleaverProg : ModProjectile
    {
            ...
            if (Hit)
            {
                ...
                if (Projectile.ai[0] <= 7)
                {
                    if (!Sound)
                    {
                        if (Bloody == 3)
                        {
                            SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver3"), Projectile.position);
                        }
                        else
                        {
                            ...
                            if (Sound == 1)
                            {
                                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver1"), Projectile.position);
                            }
                            else
                            {
                                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver2"), Projectile.position);
                            }
                        }
                        Sound = true;
                    }
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            if (Bloody == 3)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver4"), Projectile.position);
                Projectile.Kill();
            ...
            else
            {
                ...
                if (Sound == 1)
                {
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver6"), Projectile.position);
                }
                else
                {
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/CleanestCleaver5"), Projectile.position);
                }
                for (int i = 0; i < 20; i++)
                {
