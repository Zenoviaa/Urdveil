...
namespace Urdveil.Projectiles.Thrown
{
    internal class HookaramaProj : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Main.rand.NextBool(2))
            {
                SoundStyle soundStyle = new SoundStyle("Urdveil/Assets/Sounds/AssassinsKnifeHit2");
                soundStyle.PitchVariance = 0.15f;
                SoundEngine.PlaySound(soundStyle, target.position);
            }
            else
            {
                SoundStyle soundStyle = new SoundStyle("Urdveil/Assets/Sounds/AssassinsKnifeHit");
                soundStyle.PitchVariance = 0.15f;
                SoundEngine.PlaySound(soundStyle, target.position);
            }
        }
