    ...
    internal class Altride2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Timer == 1)
            {
                ...
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsKnifeHit2"), Projectile.position);
            ...
            if (Timer % 12 == 0)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlyphDust>(), Projectile.velocity * 0.1f, 0, Color.Pink, Main.rand.NextFloat(1f, 1.5f)).noGravity = true;
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Sound == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Astalaiya2"), Projectile.position);
            }
            else
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Astalaiya3"), Projectile.position);
            }

            for (int i = 0; i < 2; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlyphDust>(), Projectile.velocity.RotatedByRandom(MathHelper.ToRadians(30)) * Main.rand.NextFloat(0.2f, 1f), 0, Color.IndianRed, 1f).noGravity = true;
            }
