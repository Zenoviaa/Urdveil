...
namespace Urdveil.NPCs.Bosses.GothiviaTheSun.REK.Projectiles
{
    internal class RekFireBallProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Timer == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/RekFireballShoot"), Projectile.position);
            }
        ...
        {
            SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/RekFireballDeath"), Projectile.position);
            for (int i = 0; i < 2; i++)
            {
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height,
