...
namespace Urdveil.NPCs.Bosses.IrradiaNHavoc.Projectiles
{
    internal class IrradiaAxeProj : ModProjectile
    {
            ...
            if (Timer % 60 == 0)
            {
                if (Timer2 == 0)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center - velocityRotated * 1200, velocityRotated,
                        ModContent.ProjectileType<IrradiaAxeLaserProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Axing") { Pitch = Main.rand.NextFloat(-1f, 1f) });
                    Timer2 = 1;
                    AlphaTimer = 0;
                }
                else
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D lineTexture = ModContent.Request<Texture2D>("Urdveil/Assets/NoiseTextures/Extra_47").Value;
