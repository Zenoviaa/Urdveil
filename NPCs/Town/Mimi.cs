...
using Urdveil.Items.Weapons.Melee.Swords;
using Urdveil.Items.Weapons.Ranged;
using Urdveil.Items.Weapons.Ranged.GunSwapping;
using Urdveil.Items.Weapons.Thrown;

using Urdveil.UI.ArmorReforgeSystem;
using System;
...
using Urdveil.Items.MoonlightMagic.Weapons;
using Urdveil.Items.MoonlightMagic.Enchantments.Radiance;
using Urdveil.Items.MoonlightMagic.Enchantments.Ice;
using Urdveil.Items.MoonlightMagic.Enchantments.Nature;
using Urdveil.Items.MoonlightMagic.Enchantments.Moon;

namespace Urdveil.NPCs.Town
{
    // [AutoloadHead] and NPC.townNPC are extremely important and absolutely both necessary for any Town NPC to work at all.

    public class Mimi : VeilTownNPC
    {
        ...
        public override void AddShops()
        {
            ...
            npcShop.Add(new Item(ModContent.ItemType<IronWand>())
            {
                shopCustomPrice = 20,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<LeadWand>())
            {
                shopCustomPrice = 20,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<GoldWand>())
            {
                shopCustomPrice = 30,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<PlatinumWand>())
            {
                shopCustomPrice = 30,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<FlameLashEnchantment>())
            {
                shopCustomPrice = 15,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<TsunamiEnchantment>())
            {
                shopCustomPrice = 15,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<FloralBlastEnchantment>())
            {
                shopCustomPrice = 15,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });

            npcShop.Add(new Item(ModContent.ItemType<ShriekerEnchantment>())
            {
                shopCustomPrice = 15,
                shopSpecialCurrency = Urdveil.MedalCurrencyID
            });
