...
namespace Urdveil.Common.Bases
{
    public abstract class BaseSafunaiItem : ClassSwapItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (combo == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Safunais"), position);

            }
            if (combo == 2)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Safunais2"), position);
            ...
            {
                combo = 0;
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Safunais3"), position);
            }
