    ...
    internal abstract class BaseDiscItem : ClassSwapItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Sound == 1)
                {
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsKnifeProg"), position);
                }
                else
                {
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/AssassinsKnifeProg2"), position);
                }
            }
