...
public class ForegroundHelper : ModSystem
{
    ...
    public static Texture2D GetTexture(string name) => Urdveil.Instance.Assets.Request<Texture2D>("Gores/Foreground/" + name).Value;
    ...
    internal static void Save(TagCompound compound)
    {
        ...
        compound.Add("Urdveil:Foreground", compounds);
        compound.Add("Urdveil:PlayerLayerForeground", playerLayerCompounds);
    }

    internal static void Load(TagCompound compound)
    {
        if (compound.ContainsKey("Urdveil:Foreground"))
        {
            if (compound["Urdveil:Foreground"] is not TagCompound tag)
                return;
        ...
        if (compound.ContainsKey("Urdveil:PlayerLayerForeground"))
        {
            if (compound["Urdveil:PlayerLayerForeground"] is not TagCompound playerLayerTag)
                return;
