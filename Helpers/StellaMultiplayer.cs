...
namespace Urdveil
{
    public static class StellaMultiplayer
    {
        ...
        {
            ModPacket packet = Urdveil.Instance.GetPacket(capacity);
        ...
        public static void SpawnBossFromClient(byte whoAmI, int type, int x, int y) =>
            Urdveil.WriteToPacket(Urdveil.Instance.GetPacket(), (byte)MessageType.BossSpawnFromClient, whoAmI, type, x, y).Send(-1);
    }
}