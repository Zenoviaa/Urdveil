...
using Urdveil.Projectiles.Slashers.Vixyl;
using System.IO;
using Terraria;
...
namespace Urdveil.Items.Weapons.Melee
{
    internal class VixylPlayer : ModPlayer
    {
        ...
        public static void SendExampleDodgeMessage(int whoAmI)
        {
            ...
            ModPacket packet = ModContent.GetInstance<Urdveil>().GetPacket();
    ...
    internal class Vixyl : ClassSwapItem
    {
        ...
        public override void ModifyShootStats(Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback)
        {
            ...
            if (player.HasBuff(ModContent.BuffType<VixylDodgeBuff>()))
            {
                ...
                SoundEngine.PlaySound(new SoundStyle($"Urdveil/Assets/Sounds/SwordHoldVerlia"), position);
            }
            else if (vixylPlayer.parryCooldown <= 0 && !player.immune)
            {
                vixylPlayer.parryTimer = 18;
                //Normal slash
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/SwordSheethe"), position);
            }
        }
    }
}
