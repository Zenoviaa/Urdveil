...
using Urdveil.Projectiles.Slashers.IshNYire;
...
namespace Urdveil.Items.Weapons.Melee.Greatswords.INY
{
    ...
    public class IshNYire : ClassSwapItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            if (player.altFunctionUse == 2)
            {
                if (!player.HasBuff<VireShow>() && !player.HasBuff<IshShow>() && !player.HasBuff<YireShow>() && !player.HasBuff<ISHNYIREShow>())
                {
                    ...
                    Item.useAnimation = 30;
                    Item.useTime = 15;
                    Item.UseSound = new SoundStyle("Urdveil/Assets/Sounds/GallinLock2");
                    Item.channel = false;
                    return true;
                }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (player.altFunctionUse != 2)
            {
                ...
                if (player.HasBuff<YireShow>())
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/SwingyAr") { Pitch = Main.rand.NextFloat(-10f, 10f) }, player.Center);
        ...
        public override bool PreDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
        {
            ...
            if (!player.HasBuff<VireShow>() && !player.HasBuff<IshShow>() && !player.HasBuff<YireShow>() && !player.HasBuff<ISHNYIREShow>())
            {
                Texture2D iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Melee/Greatswords/INY/Vire").Value;
            ...
            if (player.HasBuff<IshShow>())
            {
                Texture2D iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Melee/Greatswords/INY/Ish").Value;
                spriteBatch.Draw(iconTexture, position, null, drawColor, 0f, origin, scale, SpriteEffects.None, 0);
                return false;
            }
            ...
            if (player.HasBuff<YireShow>())
            {
                Texture2D iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Melee/Greatswords/INY/Yire").Value;
                spriteBatch.Draw(iconTexture, position, null, drawColor, 0f, origin, scale, SpriteEffects.None, 0);
                return false;
            }
            ...
            if (player.HasBuff<ISHNYIREShow>())
            {
                Texture2D iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Melee/Greatswords/INY/IshNYire").Value;
                spriteBatch.Draw(iconTexture, position, null, drawColor, 0f, origin, scale, SpriteEffects.None, 0);
                return false;
            }
