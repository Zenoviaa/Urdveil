...
namespace Urdveil.Items.Weapons.Mage.Tomes
{
    ...
    internal class AstalaiyaTome : BaseMagicTomeProjectile
    {
        ...
        protected override void Shoot(Player player, IEntitySource source, Vector2 position, Vector2 velocity, int damage, float knockback)
        {
            ...
            {
                Star = 0;
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Astalaiya3"), player.position);
                type = ModContent.ProjectileType<AstalaiyaStar1>();
            }
            if (Star == 2)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Astalaiya2"), player.position);
                type = ModContent.ProjectileType<AstalaiyaStar3>();
            }
            if (Star == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/Astalaiya1"), player.position);
