...
{
    internal class ShadeHandTome : BaseMagicTomeItem
    {
        public override void SetDefaults()
        {
            base.SetDefaults();
            Item.shoot = ModContent.ProjectileType<ShadeHandTomeTome>();
            Item.shootSpeed = 10f;
        }
    }

    internal class ShadeHandTomeTome : BaseMagicTomeProjectile
    {
        private float _dustTimer;
        public override string Texture => this.PathHere() + "/ShadeHandTome";
        public override void SetDefaults()
        {
            base.SetDefaults();
            //How often it shoots
        ...
        public override void AI()
        {
            ...
            if (_dustTimer % 16 == 0)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlyphDust>(), Projectile.velocity * 0.1f, 0, Color.Purple, Main.rand.NextFloat(1f, 1.5f));
            }
        }
        ...
        protected override void Shoot(Player player, IEntitySource source, Vector2 position, Vector2 velocity, int damage, float knockback)
        {
            ...
            {
                Projectile.NewProjectile(source, position, velocity, ModContent.ProjectileType<ShadeHand>(), damage, knockback, Projectile.owner);
            }
        }
    }
}
