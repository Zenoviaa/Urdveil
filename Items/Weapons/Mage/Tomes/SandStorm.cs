...
namespace Urdveil.Items.Weapons.Mage.Tomes
{
    internal class SandStorm : BaseMagicTomeItem
    {
        ...
        {
            base.SetDefaults();
            Item.shoot = ModContent.ProjectileType<SandStormTome>();
            Item.shootSpeed = 10f;
        }
    }
    internal class SandStormTome : BaseMagicTomeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (_dustTimer % 16 == 0)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlyphDust>(), Projectile.velocity * 0.1f, 0, Color.Beige, Main.rand.NextFloat(1f, 1.5f));
            }
        }
