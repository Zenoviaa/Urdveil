...
namespace Urdveil.Items.Weapons.Ranged
{
    public class DeathShot : ClassSwapItem
    {
        ...
        public override void ModifyShootStats(Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback)
        {
            ...
            if (Sound == 1)
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/DeathShot"), player.position);
            }
            else
            {
                SoundEngine.PlaySound(new SoundStyle("Urdveil/Assets/Sounds/DeathShot2"), player.position);
            }
        }
    }
}