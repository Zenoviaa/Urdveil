...
namespace Urdveil.Items
{
    public abstract class ClassSwapItem : ModItem
    {
        ...
        public override void PostDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
        {
            ...
            if (IsSwapped)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Token").Value;
                drawOrigin = new Vector2(20, 20);
            }
            else if (AlternateClass == DamageClass.Magic)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Magic").Value;
                drawOrigin = new Vector2(18, 18);
            }
            else if (AlternateClass == DamageClass.Melee)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Melee").Value;
                drawOrigin = new Vector2(16, 20);
            }
            else if (AlternateClass == DamageClass.Ranged)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Ranger").Value;
                drawOrigin = new Vector2(18, 20);
            }
            else if (AlternateClass == DamageClass.Summon)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Summoner").Value;
                drawOrigin = new Vector2(8, 12);
            }
            else if (AlternateClass == DamageClass.Throwing)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Throwing").Value;
                drawOrigin = new Vector2(20, 20);
            }
            else if (AlternateClass == DamageClass.Generic)
            {
                iconTexture = ModContent.Request<Texture2D>("Urdveil/Items/Weapons/Transformer/Generic").Value;
                drawOrigin = new Vector2(18, 18);
            }
